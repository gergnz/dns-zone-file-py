...
def quoteField( data, field ):
    ...
    for i in xrange(0, len(data_dup)):
        data_dup[field] = '"%s"' % data_dup[field]
        data_dup[field] = data_dup[field].replace(";", "\;")
...
def processRR( data, rectype, reckey, field, template ):
    ...
    for i in xrange(0, len(data)):
        ...
        ret += '\t%s\t%s\n' % (str(rectype), "\t".join([str(data[i][rk]) for rk in reckey]))
...
def processValues( options, template ):
    """
    Replace {zone} and {datetime} in zonefile template
    """
    template = template.replace("{zone}", options.get("$origin", options.get("soa", {}).get("name", "")) )
    template = template.replace("{datetime}", datetime.date.fromtimestamp(time.time()).isoformat())
    return template.replace("{time}", str(int(round( time.time() ))))
...
def parse_line( parser, RRtok, parsed_records ):
    ...
    """
    if rectype == 'SPF':
        rrd['data'] = " ".join(rrd['data'])

    elif rectype == 'URI':
        rrd['target'] = rrd['target'].strip('"')

    elif rectype == 'TXT':
        rrd['txt'] = rrd['txt'].strip('"')
    """
    if rectype == 'PTR':
...
def make_zone_file( zonefile_data ):
    """
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 2:
        print >> sys.stderr, "Usage: %s [txt or json file]" % sys.argv[0]
        sys.exit(1)
    ...
    try:
        ...
        dat = json.loads(dat)
        zf = make_zone_file( dat )
        print zf

    except ValueError:
        # maybe it's a zone file?
        zfj = parse_zone_file( dat )
